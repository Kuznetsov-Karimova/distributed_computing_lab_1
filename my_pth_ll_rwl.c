...
void* Thread_work(void* rank) {
   ...
   for (i = 0; i < ops_per_thread; i++) {
      ...
      if (which_op < search_percent) {
         rdlock(&rwlock);  // Используем rdlock вместо pthread_rwlock_rdlock
         Member(val);
         unlock(&rwlock);  // Используем unlock вместо pthread_rwlock_unlock
         my_member_count++;
      } else if (which_op < search_percent + insert_percent) {
         wrlock(&rwlock);  // Используем wrlock вместо pthread_rwlock_wrlock
         Insert(val);
         unlock(&rwlock);
         my_insert_count++;
      } else { /* delete */
